...
export default function LandingPage() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-screen bg-[#002B45] text-white">
      ...
      <p className="text-sm font-light tracking-widest uppercase animate-pulse">Straumann Map Initializing...</p>
    </div>
  );

  if (error) return <div className="p-10">지도 로딩 에러: {error.message}</div>;

  return (
    <div className="flex flex-col min-h-screen bg-white font-sans text-[#002B45]">
      {/* 프리미엄 헤더 */}
      <header className="bg-[#002B45] text-white py-12 px-6 text-center border-b-4 border-blue-600 relative overflow-hidden">
      ...
      {/* 지도 섹션: 높이를 750px로 시원하게 고정 */}
      <main className="flex-1 relative w-full bg-slate-100">
        <Map 
          center={position} 
          style={{ width: "100%", height: "750px" }} 
          level={5}
        >
          {/* 5,103건의 데이터를 뭉쳐서 보여주는 클러스터러 */}
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-[92%] max-w-md z-30">
          <div className="bg-white/95 backdrop-blur-md shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-[2.5rem] p-7 border border-blue-50">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-1 rounded-md">Verified</span>
            </div>
            ...
            <div className="text-[11px] text-slate-400 flex items-center gap-1.5">
              <MapPin size={12} className="text-blue-300" /> 마커를 클릭하면 상세 주소를 확인하실 수 있습니다.
            </div>
          </div>
        </div>
      </main>
